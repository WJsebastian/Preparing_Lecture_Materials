...
K= 10
tensor_mem = []
tensor_mem.append(tensor_pre)
tensor_mem.append(tensor_pre)
frame_mem = []
frame_mem.append(frame)
# print(tensor_pre.shape)
# tensor_avg = torch.mean(torch.stack(tensor_mem), 0)
tensor_median = torch.median(torch.stack(tensor_mem), dim=0).values
# print(tensor_avg.shape)
# print(tensor_median.shape)
fourcc = cv2.VideoWriter_fourcc('m','p','4','v')
output = cv2.VideoWriter('output_video.mp4', fourcc, 36, (3*w,h))
while capture.isOpened():
    ...
    if ret==True:
        ...
        frame_gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        tensor_current = transform(frame_gray)
        # tensor_avg = torch.mean()
        # tensor_avg = torch.mean(torch.stack(tensor_mem), dim=0)
        tensor_median = torch.median(torch.stack(tensor_mem), dim=0).values
        diff_tensor = tensor_current - tensor_median
        # diff_tensor *= 255
        ...
        BG = cv2.resize(frame, (w *3, h), interpolation= cv2.INTER_AREA)
        BG[0:h, w:2*w] = frame
        ...
        BG[0:h, 0:w] = np.median(np.stack(frame_mem), axis=0)
        # print(np.median(np.stack(frame_mem), axis=0).shape)
        # print(frame.shape)
        BG[0: h, 2*w:3*w] = diff_frame
